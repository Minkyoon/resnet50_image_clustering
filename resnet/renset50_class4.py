...
# 전체 데이터셋을 로드합니다.
total_dataset = pd.read_csv('/home/minkyoon/crohn/csv/clam/clustering/relapse_resnet/class7_clustering.csv')

# 데이터셋을 train, validation, test로 나눕니다.
train_data, valid_data, test_data = np.split(total_dataset.sample(frac=1,  random_state=random_seed), [int(.6*len(total_dataset)), int(.8*len(total_dataset))])
...
torch.save(model_best, f'{classa}_resnet50.pt')
import matplotlib.pyplot as plt
...
# 이미지 저장 경로 설정
save_path = f'{classa}_loss_curv.png'  # 원하는 경로와 파일명으로 변경하세요.
...
model = torch.load(f'{classa}_resnet50.pt')
model.eval()
for i, (v_i, label) in enumerate(test_loader):
    # input data gpu에 올리기 
    v_i = v_i.float().to(device)
...
plt.savefig(f'{classa}_confusition.png')
plt.close()

result_string = ('Validation, Accuracy: ' + str(accuracy)[:7] + ', Sensitivity: ' 
      + str(sensitivity)[:7] + ', Specificity: ' + str(f"{specificity}")[:7] 
      + ', ROC Score: ' + str(roc_score)[:7])
with open(f'{classa}_results.txt', 'w') as f:
